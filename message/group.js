...
case 'add':{
let teks = `Hay @user jangan lupa baca desc yh semoga betah
...
Intro bebas ‿`
const welcomeText =  (chat.sWelcome|| teks).replace('user', await conn.getName(sender)).replace('@desc', groupDesc.toString() || 'unknow').replace('totalmem', groupMembers.length).replace('admin', groupAdmins.length).replace('ownergc', await conn.getName(groupOwner) || 'Tidak ada')
if (chat.welcome && !itsMe && oneMem) 
conn.sendMessage(from ,{ text : welcomeText, mentions: [mem]},{quoted: repPy})
}	
...
if (chat.welcome && !itsMe && oneMem )
conn.sendMessage(from, { text: byeText ,mentions: [mem]},{quoted:repPy})
}
break
